...
+ In order to update golden files used for tests, run the tests with `--update` flag.
  ...
  + Standard-library tests live under `corpus/lib/` (a sibling of `corpus/bal/`, discovered separately) and have no per-stage goldens at all — only `corpus/integration/lib/**.txtar`, via the `TestLibIntegration` driver in `corpus/integration_test.go`. They still run every per-stage invariant check (symbol/type resolution, CFG invariants, BIR/desugar position checks); only the golden text comparison is skipped, because a library `TestCase` has no `ExpectedPath` (`TestCase.HasStageGolden()`). `TestStageGoldensMatchCorpus` (`test_util/goldens_test.go`) keeps `corpus/bal/`'s goldens in sync with the tree in both directions — missing goldens and orphaned ones with no matching `.bal` both fail the build.
