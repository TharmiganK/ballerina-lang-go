...
### Adding or updating a stdlib package

Use the `add-stdlib-support` skill (`.agents/skills/add-stdlib-support/SKILL.md`) for the full workflow.

All `stdlib/<name>/bal/README.md` format rules — template, column constraints, Notable Behavioural Changes format, and the validation checklist — are defined in the `stdlib-readme-format` skill (`.agents/skills/stdlib-readme-format/SKILL.md`). Follow that skill exactly when creating or updating any stdlib README.
