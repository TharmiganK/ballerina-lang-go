...
// run's use case, where the rebuilt binary re-execs and takes over the
// process. interpreterRoot is the directory containing the ballerina
// go.mod; outputBinary is the destination path for the compiled native
// interpreter (typically <project>/target/bin/bal).
