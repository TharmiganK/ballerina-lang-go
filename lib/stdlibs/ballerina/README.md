...
- **An out-of-range int wraps into a `bytes`/`fixed` byte, and any int list is accepted.** A value passed to `toAvro` need not be declared `byte[]`; an `int[]` (or a bare list literal, since `anydata` carries no element type of its own) works the same way, and an int outside 0-255 (`300`, `-1`) wraps into a byte rather than being rejected — the same narrowing an out-of-range `int` gets going into an `int` schema. jBallerina's own `bytes`/`fixed` serializer only accepts a value whose declared type is exactly `byte[]`; handing it a plain `int[]`, in range or not, throws a `NullPointerException` instead.
