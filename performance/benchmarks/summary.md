...
| _Ballerina Swan Lake_ | `███████████████` | 4.881 |

**Takeaway:** Nutcracker is ready in ~0.02 s — in the instant-start group with Rust, Go and Node — while the JVM-based Swan Lake needs ~4.9 s, roughly **212× longer**.
...
| _Ballerina Swan Lake_ | `████` 38,297 | `████` 40,553 | `████` 41,796 |
...
| _Ballerina Swan Lake_ | `█` 2.68 | `█` 4.99 | `███` 11.96 |
...
| _Ballerina Swan Lake_ | `█` 8.85 | `██` 14.61 | `█████` 31.36 |
...
| _Ballerina Swan Lake_ | `███████████` 697.8 | `███████████` 695.5 | `███████████████` 971.7 |

**Takeaway:** the sharpest contrast in the test. **Nutcracker stays lean under load** (24 → 46 MB), alongside Rust and Go — while **Swan Lake climbs past 970 MB** and Spring Boot reaches ~490 MB.
...
**Nutcracker vs. Swan Lake (at 500 users):** ~**1.7×** the throughput · ~**212×** faster to start · ~**21×** less memory.

## What it means

- **Near-instant startup.** Ready in ~0.02 s with no JVM warm-up — around 212× faster than Swan Lake — and throughput then holds ~71k req/s from 100 to 500 users, with no errors.
- **Small footprint / lightweight.** Memory barely moves (24 → 46 MB), keeping company with Rust and Go, while the JVM-based runtimes (Spring Boot ~490 MB, Swan Lake ~970 MB) grow heavy under the same load.
...
| Ballerina Swan Lake | 38,297 | 2.68 | 8.85 | 1.73 | 697.8 | 356 | 0% |
...
| Ballerina Swan Lake | 40,553 | 4.99 | 14.61 | 3.01 | 695.5 | 343 | 0% |
...
| Ballerina Swan Lake | 41,796 | 11.96 | 31.36 | 6.35 | 971.7 | 384 | 0% |
