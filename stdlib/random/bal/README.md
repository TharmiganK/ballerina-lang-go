...
|---|---|---|
| Cryptographically secure random float in [0.0, 1.0) | Supported | `createDecimal()` uses Go `crypto/rand` (OS entropy pool) |
| Random integer in a caller-specified range | Supported | `createIntInRange(startRange, endRange)` uses `math/rand/v2`; range is [startRange, endRange) |
...
- **`createDecimal()` — improved entropy precision**. jBallerina delegates to `java.security.SecureRandom.nextFloat()`, which returns a Java 32-bit `float` (24 bits of mantissa) widened to a 64-bit Ballerina `float`. The Go implementation reads 53 bits from `crypto/rand`, producing a full-precision IEEE 754 `float64`. The range [0.0, 1.0) is preserved; values have higher randomness quality.

- **`createIntInRange()` — corrected range distribution**. The jBallerina formula `startRange + int(rand × (endRange−1−startRange))` never produces `endRange−1` due to an off-by-one in the original implementation. The Go version uses `math/rand/v2.Int64N(endRange−startRange) + startRange`, which correctly produces uniform values across the full `[startRange, endRange)` range per the documented specification.

- **LCG and `ballerina/time` dependency dropped**. jBallerina implements `createIntInRange` using a Linear Congruential Generator (LCG) seeded with `time:utcNow()`. The LCG is not ported because two required language features (`lock` statements and `decimal:round()`) are not yet supported in the Go interpreter. As a consequence, `ballerina/time` is also not imported — it was only used to seed the LCG state. The Go implementation replaces the LCG with `math/rand/v2`, which manages its own OS-seeded state.

- **`random:Error` and `random:ArithmeticError` not distinct**. Both error types are plain `error` aliases. Type-narrowing via `err is random:Error` works structurally but `distinct` subtyping (i.e. `err is random:ArithmeticError` being a strict subtype of `err is random:Error`) is not enforced.
